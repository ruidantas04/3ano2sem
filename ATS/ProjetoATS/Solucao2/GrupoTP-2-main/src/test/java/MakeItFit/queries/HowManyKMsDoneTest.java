...
public class HowManyKMsDoneTest {
    ...
    @Test
    void testExecuteQueryWithNullDates() {
        HowManyKMsDone kmAnalyzer = new HowManyKMsDone();
        MakeItFitDate date1 = null;
        MakeItFitDate date2 = MakeItFitDate.of(2024, 5, 10);

        assertThrows(IllegalArgumentException.class, () -> {
            kmAnalyzer.executeQuery(userManager, "test@example.com", date1, date2);
        });
    }

    @Test
    void testExecuteQueryWithNoDistanceActivities() {
        UserManager emptyManager = new UserManager();
        User noActivitiesUser = new Amateur("No Activities", 25, Gender.Female, 60, 165, 55, 2, "Rua Y", "912345678", "nodist@act.com");
        emptyManager.insertUser(noActivitiesUser);

        HowManyKMsDone kmAnalyzer = new HowManyKMsDone();
        double result = kmAnalyzer.executeQuery(emptyManager, "nodist@act.com");
