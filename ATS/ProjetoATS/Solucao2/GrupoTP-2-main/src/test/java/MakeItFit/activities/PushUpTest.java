...
public class PushUpTest {
    ...
    public void testEqualsWithDifferentObjectType() {
        ...
        String other = "Not a PushUp";
        assertNotEquals(pushUp, other, "PushUp should not be equal to object of different type.");
    }

    @Test
    public void testClone() {
        PushUp original = new PushUp(UUID.randomUUID(), MakeItFitDate.of(2024, 5, 31), 20, "Training", "PushUp", 8, 3);
        PushUp cloned = original.clone();
        assertEquals(original, cloned, "Cloned PushUp should be equal to the original.");
        assertNotSame(original, cloned, "Cloned PushUp should not be the same instance as original.");
    }

    @Test
    public void testToString() {
        PushUp pushUp = new PushUp(UUID.randomUUID(), MakeItFitDate.of(2024, 5, 31), 20, "Training", "PushUp", 10, 2);
        String str = pushUp.toString();
        assertNotNull(str);
        assertTrue(str.contains("PushUp") || str.length() > 0, "toString should return a non-empty string.");
    }
