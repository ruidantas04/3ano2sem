...
public class WhoBurnsMoreCaloriesTest {
    ...
    void testExecuteQueryWithDates() {
        WhoBurnsMoreCalories analyzer = new WhoBurnsMoreCalories();
        MakeItFitDate date1 = MakeItFitDate.of(2024, 5, 1);
        MakeItFitDate date2 = MakeItFitDate.of(2024, 5, 10);

        User result = analyzer.executeQuery(userManager, date1, date2);

        assertEquals(testUser1, result);
    }

    @Test
    void testExecuteQueryInvalidDateOrder() {
        WhoBurnsMoreCalories analyzer = new WhoBurnsMoreCalories();
        MakeItFitDate date1 = MakeItFitDate.of(2024, 5, 10);
        MakeItFitDate date2 = MakeItFitDate.of(2024, 5, 1); // date2 < date1

        assertThrows(IllegalArgumentException.class, () -> {
            analyzer.executeQuery(userManager, date1, date2);
        });
    }
