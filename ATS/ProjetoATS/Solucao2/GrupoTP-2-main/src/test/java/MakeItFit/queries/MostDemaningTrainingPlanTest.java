...
public class MostDemaningTrainingPlanTest {
    ...
    @Test
    void testExecuteQueryWhenNoTrainingPlansExist() {
        MostDemandingTrainingPlan analyzer = new MostDemandingTrainingPlan();
        TrainingPlanManager emptyTPM = new TrainingPlanManager();

        TrainingPlan result = analyzer.executeQuery(emptyTPM, userManager);

        assertNull(result, "When no training plans exist, result should be null.");
    }
