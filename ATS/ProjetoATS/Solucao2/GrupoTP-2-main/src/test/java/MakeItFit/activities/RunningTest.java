...
public class RunningTest {
    ...
    public void testDefaultConstructor() {
        ...
        assertNotNull(run);
        assertEquals(0, run.getSpeed());
    }

    @Test
    public void testToString() {
        User user = new Amateur("John", 20, Gender.Male, 70, 180, 65, 3, "Wallstreet N16", "974632836", "jonh@mail.com");
        Running run = new Running(user.getCode(), MakeItFitDate.of(2024, 4, 4), 45, "Braga em Movimento", "Running", 2300, 14.5);
        String result = run.toString();
        assertTrue(result.contains("Speed: 14.5"));
    }

    @Test
    public void testEqualsWithDifferentType() {
        Running run = new Running();
        assertNotEquals(run, new Object()); // classe diferente
    }

    @Test
    public void testClone() {
        User user = new Amateur("John", 20, Gender.Male, 70, 180, 65, 3, "Wallstreet N16", "974632836", "jonh@mail.com");
        Running original = new Running(user.getCode(), MakeItFitDate.of(2024, 4, 4), 45, "Braga em Movimento", "Running", 2300, 14.5);
        Running cloned = original.clone();

        assertNotSame(original, cloned); // objetos diferentes
        assertEquals(original, cloned);  // mas iguais em conteúdo
    }
