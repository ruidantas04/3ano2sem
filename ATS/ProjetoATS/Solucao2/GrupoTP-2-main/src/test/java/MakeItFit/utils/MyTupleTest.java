...
public class MyTupleTest {
    ...
    /**
     * Tests the compareTo method of MyTuple.
     */
    @Test
    public void testCompareTo() {
        MyTuple<Integer, String> t1 = new MyTuple<>(1, "A");
        MyTuple<Integer, String> t2 = new MyTuple<>(1, "B");
        MyTuple<Integer, String> t3 = new MyTuple<>(2, "A");
        MyTuple<Integer, String> t4 = new MyTuple<>(1, "A");

        assertTrue(t1.compareTo(t2) < 0);  // "A" < "B"
        assertTrue(t2.compareTo(t1) > 0);  // "B" > "A"
        assertTrue(t1.compareTo(t3) < 0);  // 1 < 2
        assertEquals(0, t1.compareTo(t4)); // Equal
    }

    /**
     * Tests the clone method of MyTuple.
     */
    @Test
    public void testClone() {
        MyTuple<Integer, String> original = new MyTuple<>(99, "CloneMe");
        MyTuple<Integer, String> copy = original.clone();

        assertNotNull(copy);
        assertNotSame(original, copy); // Different object
        assertEquals(original, copy);  // Same content
    }
