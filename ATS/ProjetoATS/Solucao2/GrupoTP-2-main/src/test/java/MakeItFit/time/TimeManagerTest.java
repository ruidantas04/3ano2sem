...
public class TimeManagerTest {
    ...
    @Test
    public void testDefaultConstructorAndDateAdvance() {
        TimeManager timeManager = new TimeManager(); // testa o construtor default
        assertNotNull(timeManager.getCurrentDate());

        MakeItFitDate previousDate = timeManager.getCurrentDate();
        MakeItFitDate newDate = timeManager.advanceTime(5);

        assertEquals(previousDate.plusDays(5), newDate);
        assertEquals(newDate, timeManager.getCurrentDate()); // garante que a data interna foi atualizada
    }
}